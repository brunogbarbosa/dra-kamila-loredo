...
const metrics = [
  ...
  { value: 'Escuta', label: 'VOCÊ ANTES DE TUDO' },
  { value: 'Natural', label: 'RESULTADOS QUE RESPEITAM VOCÊ' },
];

function SmileSeal() {
  const id = useId().replace(/:/g, '');
  return <div className="campaign-seal" role="img" aria-label="Seus traços, sua essência">
    <svg viewBox="0 0 180 180" fill="none" aria-hidden="true">
      ...
      <text fill="currentColor" textAnchor="middle">
        <textPath href={`#${id}-top`} startOffset="50%">SEUS TRAÇOS</textPath>
      </text>
      <text fill="currentColor" textAnchor="middle">
  ...
  return <section ref={motionRef} id="inicio" className="campaign" aria-labelledby="campaign-title">
    ...
    <div className="campaign-inner">
      <div className="campaign-copy">
        <p className="campaign-kicker">HARMONIZAÇÃO COM NATURALIDADE</p>
        <h1 id="campaign-title" aria-label="Realce o que já é único em você">
          <span className="campaign-title-line"><span>REALCE O QUE</span></span>
          <span className="campaign-title-line"><span>JÁ É ÚNICO</span></span>
          <span className="campaign-title-line"><span>EM VOCÊ</span></span>
        </h1>
        <p className="campaign-subtitle">Precisão, escuta e naturalidade para valorizar seus traços — sem apagar a sua identidade.</p>
      ...
      <figure className="campaign-portrait">
        <div className="campaign-silhouette">
          <Image className="campaign-original campaign-original-desktop" src={site.images.hero} alt="Dra. Kamila Loredo em retrato editorial" fill preload sizes="(max-width: 700px) 1px, 62vw"/>
          <Image className="campaign-original campaign-original-mobile" src={site.images.heroMobile} alt="Dra. Kamila Loredo em retrato editorial" fill preload sizes="(max-width: 700px) 100vw, 1px"/>
        </div>
      </figure>
      <SmileSeal/>
      ...
      <div className="campaign-signature"><span aria-hidden="true"/><div><p>DRA. KAMILA LOREDO</p><em>Estética avançada.<br className="campaign-signature-break"/> Naturalidade sempre.</em></div></div>
    </div>
  </section>;
}
